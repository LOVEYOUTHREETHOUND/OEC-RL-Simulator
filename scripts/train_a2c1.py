...
# Mitigate CUDA memory fragmentation if using GPU
...
# Minimal addition: import the custom Transformer features extractor
...
def parse_args():
    ...
    # Model size knobs (to control GPU memory without changing logic)
    p.add_argument("--d_model", type=int, default=128)
    p.add_argument("--nhead", type=int, default=8)
    p.add_argument("--num_layers", type=int, default=2)
    # Safety cap for rollout batch size (n_envs * n_steps)
    p.add_argument("--max_rollout_batch", type=int, default=int(os.environ.get("A2C_MAX_BATCH", 16384)), help="Cap for n_envs*n_steps to prevent OOM. If exceeded, n_steps will be reduced accordingly.")
...
def main():
    ...
    a2c_metrics_txt = os.path.join(os.path.dirname(paths["tb"]), "a2c_training_metrics.txt")
    ...
    callbacks = build_callbacks(eval_env=eval_env,
                                ...
                                a2c_metrics_txt_path=a2c_metrics_txt,
